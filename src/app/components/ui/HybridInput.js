    ...
    return (
        <div className={`relative w-full ${disabled ? "opacity-60 cursor-not-allowed" : ""}`} ref={wrapperRef}>
            ...
            {isOpen && filteredOptions.length > 0 && (
                <ul className={`absolute z-50 w-full ${placement === "top" ? "bottom-full mb-2" : "mt-2"} max-h-60 overflow-y-auto rounded-xl shadow-2xl border backdrop-blur-xl ${darkMode ? 'bg-slate-900/95 border-slate-700/50' : 'bg-white/95 border-gray-200'} custom-scrollbar`}>
