...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    // Capture the initial ratio as the "100%" baseline
    setBaseRatio(window.devicePixelRatio || 1);

    const updateZoom = () => {
      // Calculate current zoom factor relative to baseline
      const currentRatio = window.devicePixelRatio || 1;
      // We use Math.max to avoid division by zero or extreme values
  ...
  // If zoom is 1, no transform needed. 
  // Otherwise, scale down by 1/zoom to counteract the browser's scaling.
  const scale = 1 / zoom;
