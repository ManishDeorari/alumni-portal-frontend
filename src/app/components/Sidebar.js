  ...
  return (
    <>
      ...
      <nav className="hidden md:flex justify-between items-center bg-white/5 backdrop-blur-xl border-b border-white/10 text-white px-6 py-4 fixed top-0 left-0 right-0 z-50 w-full">
        ...
        <div className="flex space-x-8 items-center text-2xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
          ...
          <Link
            ...
            title="Profile"
          >
            {currentUser && currentUser.points && !["admin", "mainAdmin", "faculty"].includes(currentUser.role) && (
              <div className="flex flex-col items-end mr-1">
                <span className={`text-[9px] font-black tracking-widest uppercase ${darkMode ? getGamificationTier(currentUser?.points?.total || 0).colorClassDark : getGamificationTier(currentUser?.points?.total || 0).colorClassLight}`}>
      ...
      <nav className="flex md:hidden justify-between items-center bg-white/5 backdrop-blur-xl border-b border-white/10 text-white px-5 py-3 fixed top-0 left-0 right-0 z-50 w-full">
        <div className="text-xl font-black drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] tracking-tight">Alumni Portal</div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => window.dispatchEvent(new CustomEvent('openGlobalSearch'))}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#FAFAFA] dark:bg-[#121213] border-t border-gray-200 dark:border-white/10 px-4 py-2 z-50 flex justify-between items-center text-2xl text-gray-500 dark:text-gray-400 safe-bottom">
        ...
        <Link href="/dashboard/network" onClick={() => markSectionAsSeen("network")} className={`${pathname === "/dashboard/network" ? "text-blue-600 dark:text-blue-400" : ""} relative tap-target active:scale-90 transition-transform`}>
          <svg width="24" height="24" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6">
            <path d="M6.5 8.75V12.25L10 14.5L13.5 12.25V8.75L10 6.5L6.5 8.75ZM6.5 8.75L3.813 6.18M17.696 18.815L11.728 13.389M18.5 10.5H13.5M7.952 13.184L3.682 17.739M16.318 4.261L12.632 8.192M4.5 5.75L2.5 7L0.5 5.75V3.75L2.5 2.5L4.5 3.75V5.75ZM19.5 3.75L17.5 5L15.5 3.75V1.75L17.5 0.5L19.5 1.75V3.75ZM4.5 20.25L2.5 21.5L0.5 20.25V18.25L2.5 17L4.5 18.25V20.25ZM21 21.25L19 22.5L17 21.25V19.25L19 18L21 19.25V21.25ZM22.5 11.5L20.5 12.75L18.5 11.5V9.5L20.5 8.25L22.5 9.5V11.5Z" stroke="currentColor" strokeWidth="2.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          {pendingRequestsCount > 0 && (
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full"></span>
