...
const CreateEventRepostModal = ({ isOpen, onClose, currentUser, darkMode = false, event, setPosts, isInline = false }) => {
  ...
  const handleInputChange = (e) => {
    const { name, value } = e;
    setRepostDetails(prev => ({ ...prev, [name]: value }));
  ...
  const handleImageChange = (e) => {
    ...
    if (files.length + images.length > 4) {
      toast.error("❌ You can upload up to 4 images as proof.");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (newErrors.length > 0) {
      setErrors(newErrors);
      if (newErrors.includes("media")) toast.error("📸 Please upload at least one photo or video as proof.");
    ...
    try {
      ...
      if (result.post) {
        toast.success("🎉 Event Repost submitted! Awaiting admin approval for points.");
        if (setPosts) {
          setPosts(prev => {
        ...
        toast.error("❌ Failed to submit event repost.");
      }
    } catch (err) {
      console.error(err);
  ...
  const modalContent = (
    <div className={`p-[2px] rounded-2xl sm:rounded-[2.6rem] bg-gradient-to-tr from-green-500 to-emerald-600 w-full max-w-2xl my-auto shadow-2xl transition-all ${isInline ? "" : "max-h-[95dvh] sm:max-h-[90vh]"}`}>
      <div className={`relative w-full h-full ${darkMode ? "bg-[#121213]" : "bg-[#FAFAFA]"} rounded-xl sm:rounded-[2.5rem] overflow-hidden flex flex-col`}>
        {/* Header */}
        {!isInline && (
        <div className={`px-4 sm:px-8 py-4 sm:py-6 border-b shrink-0 ${darkMode ? "border-white/10" : "border-gray-100"} flex items-center justify-between`}>
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className={`text-[10px] uppercase tracking-widest font-bold mt-1 ${darkMode ? "text-green-500" : "text-green-600"}`}>
                  Submit proof for: {event?.name || "Event"}
                </p>
             </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 custom-scrollbar max-h-[70vh]">
          <form onSubmit={handleSubmit} className="p-4 sm:p-8 space-y-5 sm:space-y-8">
            <div className="space-y-6">
              {/* Media Section */}
              <div className={`p-[2px] rounded-3xl bg-gradient-to-r ${errors.includes("media") ? "from-red-500 to-red-600 animate-pulse" : "from-green-500 to-emerald-500"}`}>
                <div className={`p-4 border-2 border-dashed border-transparent ${darkMode ? "bg-[#121213]" : "bg-[#FAFAFA]"} rounded-[1.7rem] text-center`}>
                  <p className={`text-xs font-bold mb-4 uppercase tracking-wider ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                  ...
                  <div className="flex justify-center gap-12 mb-2">
                    <label className="cursor-pointer group flex flex-col items-center">
                      ...
                      <span className={`text-[9px] font-black uppercase tracking-widest mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Add Photo</span>
                      <input type="file" accept="image/*" multiple onChange={handleImageChange} className="hidden" />
                    </label>
                    <label className="cursor-pointer group flex flex-col items-center">
                      <span className="text-3xl block filter grayscale group-hover:grayscale-0 transition-all">🎥</span>
                      <span className={`text-[9px] font-black uppercase tracking-widest mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Add Video</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className={`text-[10px] font-black uppercase tracking-widest ${darkMode ? "text-gray-400" : "text-black"} ${errors.includes("content") ? "text-red-500" : ""}`}>Experience Description</label>
                  <EmojiPickerToggle onEmojiSelect={handleEmojiSelect} icon="😀" darkMode={darkMode} />
                </div>
                <div className={errors.includes("content") ? "p-[2px] rounded-2xl bg-red-500 shadow-lg animate-pulse" : "p-[2px] rounded-2xl bg-gradient-to-r from-green-500 to-emerald-500 shadow-sm"}>
                  <textarea
                    value={content}
                    onChange={(e) => setContent(e.target.value)}
                    rows="4"
                    placeholder="Describe what you did at the event..."
                ...
                <div className="space-y-2">
                  ...
                  <div className={errors.includes("place") ? "p-[2px] rounded-2xl bg-red-500 shadow-lg animate-pulse" : "p-[2px] rounded-2xl bg-gradient-to-r from-green-500 to-emerald-500 shadow-sm"}>
                    <input
                      type="text"
                      name="place"
                      placeholder="e.g. KP Nautiyal Auditorium"
                      value={repostDetails.place}
                      onChange={(e) => handleInputChange(e.target)}
                      className={`w-full p-4 rounded-[14px] ${darkMode ? "bg-[#121213] text-white" : "bg-[#FAFAFA] text-black"} outline-none border-none font-bold`}
                    />
                  </div>
                </div>
            ...
            <button
              ...
              className={`w-full py-4 sm:py-6 rounded-2xl sm:rounded-[2.5rem] text-xs sm:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] transition-all shadow-2xl active:scale-95 ${
                loading 
                  ...
                  : "bg-gradient-to-r from-green-600 to-emerald-700 text-white hover:shadow-green-500/25"
              }`}
            >
              {loading ? "Submitting Proof..." : "Submit Repost & Claim Points"}
            </button>
          </form>
        </div>
      </div>
...
export default CreateEventRepostModal;
