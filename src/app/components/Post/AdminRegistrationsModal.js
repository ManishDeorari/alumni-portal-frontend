...
const AdminRegistrationsModal = ({ event, isOpen, onClose, darkMode = false }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
      <div className={`relative w-full max-w-3xl ${darkMode ? "bg-slate-900 border-white/10" : "bg-white border-gray-200"} border rounded-[2rem] shadow-2xl overflow-hidden my-auto`}>
        <div className={`px-8 py-6 border-b ${darkMode ? "border-white/10" : "border-gray-100"} flex items-center justify-between`}>
          ...
          <div className="flex gap-4 items-center">
            <button 
              ...
              className="px-4 py-2 bg-green-600 text-white rounded-xl text-xs font-bold hover:bg-green-700 transition-all shadow-md active:scale-95"
            >
              📊 Download CSV
            </button>
